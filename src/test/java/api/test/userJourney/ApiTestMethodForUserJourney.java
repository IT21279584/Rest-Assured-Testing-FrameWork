...
//            Awaitility.await().pollDelay(5000, TimeUnit.MILLISECONDS).atMost(6000, TimeUnit.MILLISECONDS).until(() -> true);
            ...
            // Introduce delay using Awaitility
            introduceDelay(testCase.getDelayInSeconds());
    ...
    // Method to introduce delay based on the configuration from YAML
    private void introduceDelay(int delayInSeconds) {
        if (delayInSeconds > 0) {
            logger.info("Introducing delay of {} seconds before executing the request", delayInSeconds);
            Awaitility.await().pollDelay(delayInSeconds, TimeUnit.SECONDS).atMost(20, TimeUnit.SECONDS).until(() -> true);
        }
    }
